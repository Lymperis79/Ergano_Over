...
/// <summary>
/// Converts DateTimeOffset? ↔ string (dd/MM/yyyy).
/// Allows a plain TextBox to act as a date input — type and tab away.
/// </summary>
public class DateOffsetStringConverter : IValueConverter
{
    public static readonly DateOffsetStringConverter Instance = new();
    private const string Fmt = "dd/MM/yyyy";

    public object? Convert(object? value, Type targetType, object? parameter, CultureInfo culture)
        => value is DateTimeOffset dto ? dto.LocalDateTime.ToString(Fmt) : string.Empty;

    public object? ConvertBack(object? value, Type targetType, object? parameter, CultureInfo culture)
    {
        if (value is not string s || string.IsNullOrWhiteSpace(s)) return null;
        if (DateTime.TryParseExact(s, Fmt, culture,
                System.Globalization.DateTimeStyles.None, out var dt))
            return (DateTimeOffset?)new DateTimeOffset(dt);
        if (DateTime.TryParse(s, culture,
                System.Globalization.DateTimeStyles.None, out var dt2))
            return (DateTimeOffset?)new DateTimeOffset(dt2);
